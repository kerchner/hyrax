  ...
  module Valkyrie
    class QueryService
      ...
      def count_all_of_model(model:)
        ActiveFedora::Base
          .where(has_model_ssim: [model_class_for(model).to_s, model.new.internal_resource.to_s])
          .count
      end
      ...
      def find_by_alternate_identifier(alternate_identifier:, use_valkyrie: true)
        alternate_identifier = ::Valkyrie::ID.new(alternate_identifier.to_s) if
          alternate_identifier.is_a?(String)
